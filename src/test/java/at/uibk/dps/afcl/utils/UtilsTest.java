...
public class UtilsTest {
	...
	/**
	 * Creates an example of a simple workflow consisting of an atomicFunction and a
	 * parallelFor loop.
	 *
	...
	private Workflow getSimpleWorkflow() {
		...
		ParallelFor parallelFor = new ParallelFor("parallelFor", null, Arrays.asList("10", "collectionName"),
	...
	/**
	 * Creates an example of a simple workflow which is not valid according to the
	 * schema.
	 *
	...
	public void writeReadJsonTest() {
		...
		File schema = new File(
				Objects.requireNonNull(getClass().getClassLoader().getResource("schema.json")).getFile());
